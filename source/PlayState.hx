...
class PlayState extends MusicBeatState
{
	...
	private var canResync:Bool = true;
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'trunk':
				dialogue = CoolUtil.coolTextFile(Paths.txt('trunk/Dialogue'));
			case 'warning':
				dialogue = CoolUtil.coolTextFile(Paths.txt('warning/Dialogue'));
			case 'revolution':
				dialogue = CoolUtil.coolTextFile(Paths.txt('revolution/Dialogue'));
			case 'synergy':
				dialogue = CoolUtil.coolTextFile(Paths.txt('synergy/Dialogue'));
		}
			...
			case 'park1':
				{
						defaultCamZoom = 0.7;
						curStage = 'park1';
						var hallowText = Paths.getSparrowAtlas('tree_and_bg','shared');
			...
			case 'park-1':
				{
					...
					halloweenBG = new FlxSprite(-600, -200).loadGraphic(Paths.image('grass', 'shared'));
					halloweenBG.antialiasing = true;
					halloweenBG.scrollFactor.set(0.9, 0.9);
					add(halloweenBG);	
				}
	...
	function startSong():Void
	{
		...
		if (SONG.song.toLowerCase() == 'synergy' && isStoryMode)
			FlxG.sound.music.onComplete = endSynergy;
		else
		...
		if (generatedMusic)
			{
				...
				{	
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
						...
						else if (SONG.song.toLowerCase() == 'revolution')
							switch (Math.abs(daNote.noteData))
						{
							case 2:
								dad.playAnim('singUP' + altAnim, true);
								gf.playAnim('uhoh', false);
							case 3:
								dad.playAnim('singRIGHT' + altAnim, true);
								gf.playAnim('uhoh', false);
							case 1:
	...
	function endSynergy():Void {
		...
		FlxG.sound.music.stop();
		vocals.stop();
		var endDialogue = CoolUtil.coolTextFile(Paths.txt('synergy/EndDialogue'));
