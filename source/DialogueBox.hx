...
class DialogueBox extends FlxSpriteGroup
{
	...
	var portraitLefttttt:FlxSprite;
	...
	public function new(talkingRight:Bool = true, ?dialogueList:Array<String>)
	{
		...
		portraitLefttttt = new FlxSprite(-100, 40);
		portraitLefttttt.frames = Paths.getSparrowAtlas('MattP');
		portraitLefttttt.animation.addByPrefix('enter', 'Boyfriend portrait enter', 24, false);
		portraitLefttttt.updateHitbox();
		portraitLefttttt.flipX = true;
		portraitLefttttt.scrollFactor.set();
		add(portraitLefttttt);
		portraitLefttttt.visible = false;
	...
	function startDialogue():Void
	{
		...
		switch (curCharacter)
		{
			...
			case 'matt':
				...
				portraitRightbf.visible = false;
				swagDialogue.sounds = [FlxG.sound.load(Paths.sound('TextDialog'), 0.6)];
				if (!portraitLefttttt.visible)
				{
					portraitLefttttt.visible = true;
					portraitLefttttt.animation.play('enter');
					swagDialogue.sounds = [FlxG.sound.load(Paths.sound('TextDialog'), 0.6)];
				}
			case 'gf':
				portraitLeft.visible = false;
